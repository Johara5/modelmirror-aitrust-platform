...
const Dashboard: React.FC<DashboardProps> = ({ analysis, history }) => {
  const trustScore = analysis ? analysis.trustScore : MOCK_HEALTH_DATA.overallTrustScore;
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden group">
          <div className="flex items-center justify-between mb-4 relative z-10">
            <span className="text-slate-500 text-sm font-medium uppercase tracking-wider">System Trust Score</span>
            <ShieldCheck className="text-indigo-600" size={20} />
          </div>
          <div className="text-4xl font-black text-slate-900 relative z-10">{trustScore}%</div>
          <div className={`mt-2 text-sm font-bold relative z-10 flex items-center gap-1.5 ${analysis ? 'text-indigo-600' : 'text-green-600'}`}>
            {analysis ? <><Zap size={14} fill="currentColor" /> Live Feed Active</> : '↑ 2.4% from last week'}
          </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="text-4xl font-black text-slate-900">
            {history ? history.filter(p => p.anomalyDetected).length : MOCK_HEALTH_DATA.driftAlerts}
          </div>
          <div className="mt-2 text-sm text-slate-500 font-medium">Session findings</div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="text-4xl font-black text-slate-900">
            {history ? history.length : MOCK_HEALTH_DATA.totalAudits.toLocaleString()}
          </div>
          <div className="mt-2 text-sm text-slate-500 font-medium">Completed units</div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="mt-2 text-sm text-slate-500 font-medium">Stability Index</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="h-[350px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                />
                <Area type="monotone" dataKey="confidence" stroke="#4f46e5" strokeWidth={3} fillOpacity={1} fill="url(#colorConf)" name="Confidence" animationDuration={300} isAnimationActive={!history} />
                <Area type="monotone" dataKey="errors" stroke="#f43f5e" strokeWidth={2} fillOpacity={0.05} strokeDasharray="5 5" fill="#f43f5e" name="Drift/Error" animationDuration={300} isAnimationActive={!history} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col">
            ...
            {analysis ? (
              <div className="space-y-3">
                <div className="p-3 bg-indigo-50 border border-indigo-100 rounded-lg">
                  <p className="text-[10px] font-black text-indigo-700 uppercase mb-1">Mirror Snapshot</p>
                  <p className="text-xs text-indigo-900 leading-snug">{analysis.explanations.simple}</p>
                </div>
          ...
          <button className="w-full mt-6 py-3 text-xs font-bold text-slate-500 bg-slate-50 border border-slate-200 rounded-xl hover:bg-slate-100 transition-all uppercase tracking-widest">
            Full Session Log
          </button>
        </div>
      </div>
